...
s=pd.read_csv(os.path.join(sources_path,'usgs-sfb-data-processed.csv'),
              parse_dates=['Date'])
...
field=pd.read_csv(os.path.join(sources_path,'EMP_Field_updated.csv'),
...
lab0 =pd.read_csv(os.path.join(sources_path,'EMP_Lab_updated.csv'),
                  parse_dates=['SampleDate'],
...
lab1 = pd.DataFrame({'SampleDate':lab1['SampleDate'].values, 'StationCode': lab1['StationCode'].values, 'Depth': lab1['Depth'].values, 'ConstituentName': lab1['ConstituentName'].values, 'Result': lab1['Result'].values.astype(float)})
